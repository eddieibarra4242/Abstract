...
{
	vec3 lightDirection = normalize(worldPos - light.light.position);
	float sFactor = dot(lightDirection, light.direction);

	vec4 color = vec4(0,0,0,0);

	if(sFactor > light.cutOff)
	{
		color = calcPointLight(light.light, normal, worldPos) *
				(1.0 - (1.0 - sFactor) / (1.0 - light.cutOff));
	}
...
void main()
{
    ...
    vec4 light = vec4(calcSpotLight(R_spotLight, normal, worldPos).xyz, 1);
